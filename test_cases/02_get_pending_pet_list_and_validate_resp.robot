*** Comments ***
#####################################################################################################
Objective:- The Aim of the test case to get the list of pending pet list and validate the response
...    via GET Api request
Author:- Gurubasava M
######################################################################################################
...
*** Variables ***
${EXP_STATUS_CODE}    ${200}
${EXP_PET_STATUS}    pending

*** Test Cases ***
Send API Request To Get The Pet List :
    ${response} =    Get Pet Information Via GET Req   ${PETSTORE_URL['url']}    ${PETSTORE_URL['uri']}status=pending
